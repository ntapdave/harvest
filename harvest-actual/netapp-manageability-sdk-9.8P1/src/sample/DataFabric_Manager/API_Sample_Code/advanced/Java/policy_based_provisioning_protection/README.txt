...
1. RESOURCE POOL
   1a. Create a primary resource pool - "Primary_pool" and add storage system 
       ...
       a. Create resource pool "Primary_Pool"
	      C:\> java resource_pool DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD create POOL_NAME
		  
	      For e.g:
	      C:\> java resource_pool DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD  create Primary_Pool
	   
       b. Add members (STORAGE_SYSTEM_NAME) into the resource pool (Primary_Pool) 
	      with resource-labels (GOLD).  Repeat the below step for each member that 
	      you want to add.  Make sure you add appropriate labels.
	   
          C:\> java resource_pool DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD member add POOL_NAME STORAGE_SYSTEM_NAME RESOURCE_LABEL   
		   
          For e.g:
          C:\> java resource_pool DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD member add Primary_Pool fas6fc GOLD   
          C:\> java resource_pool DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD member add Primary_Pool fas3sata SILVER   
	...
	1c. List resource pools
        ...
        a. List all resource pools 
			
           C:\> java resource_pool DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD list
			
        b. List members in a particular resource pool
           C:\> java resource_pool DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD member list POOL_NAME
			
           For e.g:
           C:\> java resource_pool DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD member list Primary_Pool
...
2. POLICIES
   2a. Provisioning Policy
       ...
       a. List all provisioning policies
          ...
          C:\> java policy DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD list prov
		  
       b. Create a new provisioning policy with resource labels.  This policy is dedicated 
          to create or provision new storage from a resource pool with members tagged with
          ...
          C:\> java policy DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD create prov POLICY_NAME POLICY_TYPE RESOURCE_LABEL default
		
          For e.g:
          C:\> java policy DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD create prov my_nas_policy nas GOLD default
   ...
   2b. Protection Policy
	...
	a. List all protection policies
	   ...
	   C:\> java policy DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD list dp mirror
...
3. MULTISTORE 
   3a. Multistores enable creation of a multi-tenant environment on NetApp storage systems.  
       ...
       resource pool where data resides.  But, if you are looking to setup a multi-tenant 
       ...
       a. Create a multistore with NAS (nfs & cifs) protocols enabled.
          ...
          C:\> java multistore DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD create MULTISTORE_NAME IP_ADDRESS PROTOCOL STORAGE_SYSTEM_OR_RPOOL
		  
	      For e.g:
	      C:\> java multistore DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD create fas6ms1 10.10.10.1 nas fas6fc
       ...
       b. Setup the Multistore 
	      C:\> java multistore DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD setup MULTISTORE_NAME INTERFACE IP_ADDRESS NETMASK
		  
	      For e.g:
	      C:\> java multistore DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD setup fas6ms1 e0a 10.10.10.1 255.255.255.0
		  
		  
4. DATASET
   4a. Create a dataset.  
       C:\> java dataset DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD create DATASET_NAME MULTISTORE_TO_ATTACH
	   
       For e.g:
       C:\> java dataset DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD create Oil_Pri_data fas6ms1
   ...
   4b. Add protection and provisioning policies.  The resource pool list depends on the policy chosen. 
       ...
       C:\> java dataset DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD update DATASET_NAME PROV_POLICY PROT_POLICY RPOOL1 [RPOOL2]
 
       For e.g:
       C:\> java dataset DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD update Oil_Pri_data my_nas_policy Mirror Primary_Pool Backup_Pool
   ...
   4c. With this setup complete, any storage that is provisioned from this dataset will be automatically 
       ...
       C:\> java dataset DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD provision DATASET_NAME MEMBER_TO_PROV SIZE 
	   
       For e.g:
       C:\> java dataset DFM_SERVER DFM_ADMIN DFM_ADMIN_PWD provision Oil_Pri_data Docs_lib 5000000000
