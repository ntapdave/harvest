...
function PrintUsage
{
	...
	write $("`t -o {originator_id} `t Pass Originator Id");
        ...
        write $("`n  Note:");
        write $("        Use server type 'dfm' (-t dfm) for Active IQ Unified Manager (formerly OnCommand Unified Manager) server version 5.2 or earlier.");
        write $("        Use server type 'ocum' (-t ocum) for Active IQ Unified Manager (formerly OnCommand Unified Manager) server version 6.0 or later.`n");
