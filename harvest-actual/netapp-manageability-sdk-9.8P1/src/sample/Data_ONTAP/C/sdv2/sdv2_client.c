...
static void sd_init_start_args(sd_start_args_t *sd_start_args);
static int32_t sd_validate_start_args(sd_start_args_t *sd_start_args);
 ...
 */
int32_t
static sd_validate_start_args(sd_start_args_t *sd_start_args)
{
...
/*	opterr = 0; //suppress getopt's output */
	...
	//set defaults
	sd_init_start_args(sd_start_args);

	while ((opt = getopt_long(argc, argv, "ACFIa:b:d:f:m:n:p:r:s:t:u:v:", sd_long_options, NULL)) != -1)
	{
		switch (opt) {
			case 'A':
	...
	return sd_validate_start_args(sd_start_args);
}
...
static void print_usage()
{
	fprintf(stderr, "USAGE: sdv2-client -f <IP> [-s <VSERVERNAME>]-v <VOLNAME> -d <DIFF_SNAP> -m <MAX_DIFFS>\n");
	...
	fprintf(stderr, "		IP	  : Vserver Data IP address. If passing Cluster Management IP address, set VSERVERNAME\n");
	fprintf(stderr, "			    using '-s' option as mentioned above.\n");
	fprintf(stderr, "	USERNAME, PASSWORD: Vserver/Cluster credentials depending on which IP address is used above.\n");
}

/*
 * Get the Vserver Data Lif IP addr
 */
static uint32_t get_vserver_ip(char *clus_ip, char *uname, char *pwd,
			       char *vserver_name, char **vserver_ip)
{
	na_server_t *server = NULL;
        na_elem_t *requestElem = NULL, *query = NULL;
        na_elem_t *info= NULL, *proto = NULL;
	na_elem_t *out = NULL, *attrs = NULL, *netint = NULL;
        char na_err[256];
        na_elem_iter_t iter;
	uint32_t error = 0;

	if (!na_startup(na_err, sizeof(na_err))) {
		DBG_PRINTF("Error in na_startup: %s\n", na_err);
		error = EPROTO;
		goto done;
	}
	// Initialize connection to server, and request version 1.15 of the API set
	server = na_server_open(clus_ip, 1, 15);
	if(!server) {
		DBG_PRINTF("Error in na_server_open\n");
		error = EPROTO;
		goto errshutdown;
	}

	na_server_adminuser(server, uname, pwd);

	proto = na_elem_new("data-protocols");
	if (!proto) {
		error = ENOMEM;
		goto errclose;
	}
	na_child_add_string(proto, "data-protocol", "nfs");

	info = na_elem_new("net-interface-info");
	if (!info) {
		error = ENOMEM;
		na_elem_free(proto);
		goto errclose;
	}
	na_child_add_string(info, "vserver", vserver_name);
	na_child_add_string(info, "role", "data");
	na_child_add(info, proto);

	query = na_elem_new("query");
	if (!query) {
		error = ENOMEM;
		na_elem_free(info);
		goto errclose;
	}
	na_child_add(query, info);

	requestElem = na_elem_new("net-interface-get-iter");
	if (!requestElem) {
		error = ENOMEM;
		na_elem_free(query);
		goto errclose;
	}
	na_child_add(requestElem, query);

	out = na_server_invoke_elem(server, requestElem);
	if (!out) {
		error = EPROTO;
		na_elem_free(requestElem);
		goto errclose;
	}
	...
	if (na_child_get_int(out, "num-records", 0) == 0) {
		DBG_PRINTF("Vserver specified is not found/supported.\n");
		error = EINVAL;
		goto errclean;
	}
	DBG_PRINTF("Number of records: %d\n", na_child_get_int(out, "num-records", 0));
	/* If we have multiple records matching the above query, lets pick the first one. */
	attrs = na_elem_child(out, "attributes-list");
	iter = na_child_iterator(attrs);
	if ((netint = na_iterator_next(&iter)) != NULL) {
		*vserver_ip = strdup(na_child_get_string(netint, "address"));
	}

	DBG_PRINTF("Vserver IP: %s\n", *vserver_ip);

errclean:
    // free the ZAPI resource
    na_elem_free(requestElem);
    na_elem_free(out);
...
int main(int argc, char **argv)
{
	...
	char *connect_ip = NULL, *vserver_ip = NULL;
	...
	status = sd_validate_start_args(&sd_start_args);
	if (status != SD_SUCCESS) {
		err_origin = ERR_ORIGIN_SDCLNT;
		goto errdone;
	}
	...
	/*
	 * If vserver name is specified, that means the ip_addr is cluster_mgmt IP
	 * So, call a ZAPI to get the vserver ip addr and pass it to sd_rpc_connect()
	 */
	if (sd_start_args.vserver != NULL) {
		error = get_vserver_ip(sd_start_args.ip_addr, sd_start_args.username,
				       sd_start_args.password, sd_start_args.vserver,
				       &vserver_ip);
		if (error != 0) {
			printf("Failed to get vserver data IP address\n");
			err_origin = ERR_ORIGIN_ZAPI;
			status = SD_ERROR;
			goto errdone;
		}
		if (vserver_ip == NULL) {
			printf("Invalid vserver data IP address\n");
			err_origin = ERR_ORIGIN_ZAPI;
			status = SD_ERROR;
			goto errdone;
		}
		...
		connect_ip = sd_start_args.ip_addr;
	}
