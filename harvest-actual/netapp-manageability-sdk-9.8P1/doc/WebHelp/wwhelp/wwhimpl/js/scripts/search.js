...
function  WWHSearch_Object()
{
  ...
  this.fTypeCheck              = WWHSearch_TypeCheck; // Valee added this line
  ...
  if (this.mbPanelInitialized)
  {
    ...
    else
    {
      ...
      if (this.mSavedSearchWords.length === 0)
      {
        ...
        //Valee added the below style for h3
        HTML.fAppend("<h3 style='font-family:Arial,Tahoma;font-size:10pt;font-weight:normal'>" + WWHFrame.WWHJavaScript.mMessages.mSearchDefaultMessage + "</h3>\n");
...
// Valee added the following function
function WWHSearch_TypeCheck(){
	
  if ((WWHFrame.WWHHandler.fIsReady()) && (this.mSearchState == null)){
    VarPanelNavigationFrame = eval(WWHFrame.WWHHelp.fGetFrameReference("WWHPanelNavigationFrame"));
    ...
    if(SearchForm.elements["WWHSearchType"][1].checked){
    	var strURL = "http://mysupport.netapp.com/search?esSubmit=&access=p&output=xml_no_dtd&site=gs&client=gs&proxystylesheet=gs&getfields=*&filter=p&requiredfields=cot%3Aproduct+documentation&q=";
    	strURL = strURL + NewSearchWords;
    	var objW = window.open(strURL, "oWin");
			return false;    	
    }
  }
}
...
function  WWHSearch_DisplaySearchForm()
{
  ...
  // Valee added the following lines
  HTML += "<form name=\"WWHSearchForm\" onsubmit=\"WWHFrame.WWHSearch.fSubmit();\">\n";
  HTML += "<nobr>\n";
  ...
  //HTML += "</nobr>\n";
	//Valee added the following lines
  HTML += "<input tabindex=\"1\" type=\"text\" name=\"WWHSearchWordsText\" size=\"30\" value=\"" + WWHStringUtilities_EscapeHTML(this.mSavedSearchWords) + "\" onkeydown=\"WWHFrame.WWHHelp.fIgnoreNextKeyPress((document.all||document.getElementById||document.layers)?event:null);\">\n";
  HTML += "<input tabindex=\"2\" type=\"submit\" value=\"" + WWHFrame.WWHJavaScript.mMessages.mSearchButtonLabel + "\" onclick=\"return WWHFrame.WWHSearch.fTypeCheck();\">\n<br>";
  HTML += "<input tabindex=\"3\" type=\"radio\" name=\"WWHSearchType\" value=\"WebHelp\" checked=\"checked\"><span style=\"font-family:arial;font-size:10pt;\">Product Help</span></input>\n";
  HTML += "<input tabindex=\"4\" type=\"radio\" name=\"WWHSearchType\" value=\"Support\"><span style=\"font-family:arial;font-size:10pt;\">Support Site</span></input>\n";
