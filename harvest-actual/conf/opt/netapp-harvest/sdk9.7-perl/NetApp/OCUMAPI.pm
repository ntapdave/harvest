...
# $Id$
# 
# Copyright (c) 2020 Network Appliance, Inc.
# All rights reserved.
# 
# This file was generated at Mar 5, 2020 7:27:42 AM
##
...
OCUMAPI - Contains the definitions and description of API Bindings for Active IQ Unified Manager (formerly OnCommand Unified Manager) server 6.0 or later

=head1 SYNOPSIS

	 my $s = NaServer->new($server, 1, 0); # create NaServer (server context)
	 $s->set_admin_user('admin', 'password'); # provide username and password
	 $s->set_server_type('OCUM'); # set the server type to OCUM for Active IQ Unified Manager (formerly OnCommand Unified Manager) 6.0 or later
...
NetApp Manageability SDK 9.6 provides support for Perl API bindings for both Data ONTAP APIs and Active IQ Unified Manager (formerly OnCommand Unified Manager) APIs.
The Perl API bindings libraries contain interfaces to establish a connection with either the Data ONTAP server or the Active IQ Unified Manager (formerly OnCommand Unified Manager) server.
By using these libraries, you can create Perl applications to access and manage the Data ONTAP server or Active IQ Unified Manager (formerly OnCommand Unified Manager) server.

NetApp Manageability SDK 9.6 Perl API bindings provide a runtime library NaServer.pm, which is available at <installation_folder>/lib/perl/NetApp.
...
Each binding can be called as a subroutine of NaServer module which in turn invokes the corresponding Data ONTAP or Active IQ Unified Manager (formerly OnCommand Unified Manager) API.
...
=head2 event_class_iter



=begin html

Iterate and list event-classes.
...
=item * I<event-generation-enabled>  =>   B<boolean, optional>

=begin html

<br>If false, event generation is disabled for this event class.
...
=item * I<impact-level>  =>   B<event-impact-level, optional>

=begin html

<br>Impact level of generated event.
...
=item * I<severity>  =>   B<event-severity, optional>

=begin html

<br>Severity of generated event.
...
=item * I<records>  =>   B<L<"event-class-info">[], optional>
...
=head2 event_class_modify



=begin html

Modify one or more properties of an event class. <P> Error Conditions: <ul> <li> ENOSUCHEVENTCLASS - If no matching id or class is found. </ul>
...
=item * I<event-class-name-or-id>  =>   B<object-name-or-id>

=begin html

<br>Id or unique-name of the event-class.
...
=item * I<event-generation-enabled>  =>   B<boolean>

=begin html

<br>If false, event generation will be disabled for this class.
...
<br>Snaplock-type of the new resource pool. The default value is non-snaplock. Possible values - "compliance", "enterprise" or "non-snaplock".
...
<br>Modified snaplock-type of the resource pool. The default value is non-snaplock. Possible values - "compliance", "enterprise" or "non-snaplock".
...
<br>A string that adheres to the following regular expression: [1-9][0-9]*\.[0-9]+\.* The first number is the major version. The second number is the minor version. Example: 7.1RC1
...
=item * I<is-autoSize-enable>  =>   B<boolean, optional>

=begin html

<br>Enable autoSize for the volume. By default true.
...
=item * I<is-compression-enable>  =>   B<boolean, optional>

=begin html

<br>Enable compression on the volume. By default false. If true, is-dedupe-enable flag should be true. Otherwize fails as EINVALIDINPUT error.
...
=item * I<is-dedupe-enable>  =>   B<boolean, optional>

=begin html

<br>Enable dedupe for the volume. By default false.
...
=item * I<is-thin-provision-enable>  =>   B<boolean, optional>

=begin html

<br>If true, secondary storage is provisioned as thin. By default true.
...
=item * I<resource-pools>  =>   B<resource-key[]>

=begin html

<br>List of resource pools used to provision storage.
...
=item * I<size>  =>   B<string>

=begin html

<br><b>Flexible</b> volumes only. The initial size of the new flexible volume. The format to use is: <number> kb|mb|gb|tb or <number> KB|MB|GB|TB where 'kb'/'KB' means kilobytes, 'mb'/'MB' means megabytes, 'gb'/'GB' means gigabytes, and 'tb'/'TB' means terabytes. If the trailing unit character doesn't appear, then <number> is interpreted as the number of bytes desired.
...
<br>Name of the volume to create. The volume name can contain letters, numbers, and the underscore character (_), but the first character must be a letter or an underscore. In Data ONTAP Cluster-Mode, the volume names must be unique within a Vserver.
...
=item * I<vserver-resource-key>  =>   B<resource-key, optional>

=begin html

<br>Vserver that is used to provision storage. If not provided selects the vserver from Storage Virtual Machine Associations.
...
<br>Vserver that is used to provision storage.
...
=item * I<aggregate-snaplock-type>  =>   B<string, optional>

=begin html

<br>Snaplock-type of the aggregate. Possible values - "compliance", "enterprise" or "non-snaplock".
...
=item * I<is-cft-precommit>  =>   B<boolean>

=begin html

<br>Specifies whether a given aggregate is in pre-commit state or not.
...
<br>The current status of the device. Possible values are "online", "ok", "failed", "erasing", "erased", "offline_failed" and "offline_threshold".
...
=item * I<is-network-compression-enabled>  =>   B<boolean, optional>

=begin html

<br>When this flag is set to true, network compression is enabled during snapmirror restore and if set to false network compression is disabled. If using this parameter, is-use-snapmirror-restore must also be set to true. Otherwise, EINVALIDINPUT is returned. This flag is available for source and destinations running Data ONTAP 8.3 or later. The default value for this parameter is 'false'.
...
=item * I<is-use-snapmirror-restore>  =>   B<boolean, optional>

=begin html

<br>This optional parameter specifies whether to use Data ONTAP's snapmirror-restore API for restoring the selected files or LUNs. If using this parameter, source-path parameter must also be specified whereas destionation-path is optional. The snapmirror-restore API creates a temporary SnapMirror restore relationship. For this reason the destination volume cannot be the destination volume of another SnapMirror relationship. Also the destination volume should be of the type RW Otherwise, EINVALIDINPUT is returned. When this flag is not specified or false, the snapmirror-restore API is not used for restore. The default value for this parameter is 'false'. When this flag is set to true, use-snapshot-restore-volume flag should not be specified or if specified should be set to false. Otherwise, EINVALIDINPUT is returned. This flag is available for source and destinations running Data ONTAP 8.3 or later. Otherwise, EINVALIDINPUT is returned. The default value for this parameter is 'false'.
...
<br>Path within the Snapshot copy of the storage object that is being restored. If use-snapshot-restore-volume flag is specified, then the source-path element must not be specified. Source path is relative to the Snapshot copy location and it can be a directory or a file. If a directory is specified, the files and subdirectories of that directory are restored. Leading '/' is optional in the source-path. Examples of the source-path are "dir1/dir2" or "/dir1/dir2" or "/dir1/dir2/file1". If the source-path is "/dir1/dir2", it is interpreted as "&lt;source-volume&gt;/.snapshot/&lt;snapshot&gt;/dir1/dir2". '/' must be used in place of '\' when restoring to or from a Windows path. If is-use-snapmirror-restore flag is specified, then the following applies: The source-path element must be specified. In this case source path can only specify a file or a LUN of the Snapshot copy. Otherwise EINVALIDINPUT is returned. Upto a maximum of 8 files or LUNs can be restored simultaneously and this multiple file restore option within snapmirror-restore zapi is available on Data ONTAP 8.3 or later. Otherwise, EINVALIDINPUT is returned. Users can provide multiple source-paths as part of multiple dp-restore-request-infos.
...
=item * I<category>  =>   B<event-category, optional>
...
=item * I<event-class-type>  =>   B<string>

=begin html

<br>Event class type. Possible values: 'built_in', 'custom'.
...
=item * I<impact-area>  =>   B<event-impact-level>
...
=item * I<impact-level>  =>   B<event-impact-level>
...
<br>Event service will not drop duplicate events of this event class if is-allow-duplicates is true. Event is duplicate if it has same event-name as previous event with same event-class and the same event-source.
...
<br>Event service keeps multiple current events of this event-class for each event-source.
...
=item * I<pretty-name>  =>   B<string>

=begin html

<br>Pretty name of the event class.
...
=item * I<severity>  =>   B<event-severity>
...
=item * I<unique-name>  =>   B<string>

=begin html

<br>Name of the event class.
...
Impact area of the event. <p> Possible values are: <ul> <li>availability <li>capacity <li>configuration <li>performance <li>protection </ul>
...
<br>Type or class to which the event belongs to.
...
Severity of the event. <p> Possible values are: <ul> <li>normal <li>information <li>warning <li>error <li>critical </ul>
...
<br>Impact level of the event name.
...
The state of the job. The possible values are: <ul> <li> running <li> completed <li> aborting <li> aborted </ul>
...
The state of the task. The possible values are: <ul> <li> waiting <li> running <li> completed </ul>
...
=item * I<is-dedupe-running>  =>   B<boolean>

=begin html

<br>Returns true if the specified volume is currently running dedupe, false otherwise.
...
=head2 provision-attributes

=begin html

Contains all provisioning attributes of secondary storage.
...
=item * I<is-thin-provision>  =>   B<boolean, optional>

=begin html

<br>The flag to provision secondary storage as thin or thick. If true, secondary storage is provisioned as thin. By default true.
...
<br>The resource key of storage-service-workflow is obtainable from storage-service-workflow-list-info API.
...
<br>Data ONTAP relationship between two nodes. Possible values are: 'mirror', 'vault', 'version_flexible_mirror'.
...
<br>Indicates whether the network compression should be enabled for transferring snapshots across snapmirror relationships, if set to true it will be enabled. The default value is false. NOTE: Inorder to take the effect of this parameter, both source and destination volumes of snapmirror relationship should be running Data ONTAP 8.3 or later otherwise this parameter has no effect.
...
<br>Specifies the upper bound for each relationship, in kilobytes per second, at which data is transferred between ONTAP clusters. The default is NULL, which indicates that the value set on ONTAP will be used. If a value is specified, then that value is used as the data transfer rate. The max-transfer-rate option does not affect relationships confined to a single cluster.
...
=item * I<provision-attributes>  =>   B<L<"provision-attributes">, optional>

=begin html

<br>All Provisioning attributes for secondary storage. Should not be specified for the root node, and optional for non-root nodes.
...
<br>Provisioning workflow that is used to provision secondary storage. Should not be specified for the root node, and optional for non-root nodes. The name of storage-service-workflow is obtainable from storage-service-workflow-list-info API. This will be deprecated in future releases. Need to use "provision-attributes".
...
<br>Whether the volume is in pre-commit state or not.
...
<br>Snaplock-type of the volume. Possible values - "compliance", "enterprise" or "non-snaplock".
