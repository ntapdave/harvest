...
=head2 set_server_type($type)
  ...
  or Active IQ Unified Manager (formerly OnCommand Unified Manager) server.
...
sub invoke_elem ($) {
	...
	my $host = "Host";
#differentitating IPV4 and IPV6 address
        if ( $server =~ /:/ ) {
...
sub verify_server_certificate ($$$) {
	...
	if ($x509_name) {
		...
		my $newcertname=$cert_name;
        $newcertname=~s/^.{2}//g;
		if (index($host_name,$newcertname)==-1) {
		...
		if ($osType eq 'MSWin32') {
			...
			if ($osType eq 'linux') {
				if (-e "/etc/SuSE-release") {
					...
					chomp($osName = `head -n 1 /etc/redhat-release`);
				} else {
					$osName = 'Linux';
				}
				if ($osName =~ m/(.*?) \(.*?\)/) {
